...
export default function DayTabs({ days, activeDay, onSelectDay }) {
  return (
    <div className="sticky top-[57px] z-20 bg-white/90 backdrop-blur-md border-b border-slate-200 py-2.5 px-4 shadow-xs">
          ...
          return (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-xs font-bold transition-all whitespace-nowrap flex items-center gap-1.5 ${
                isSelected
                  ? 'bg-secondary-500 text-white shadow-md ring-2 ring-secondary-950 scale-102'
                  : 'bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-200'
              }`}
            >
              <span>Day{d.day}</span>
              <span className={isSelected ? 'text-secondary-950 font-extrabold' : 'text-slate-500'}>
                {d.date}
              </span>
            </button>
          )
